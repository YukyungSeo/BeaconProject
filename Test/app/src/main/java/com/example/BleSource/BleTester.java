...
public class BleTester {
    ...
    public void BleTestFunc(String myID,TextView datatxt) {  //메인 기능 함수
    ...
    private void checkPhonesNearBeacon(String[] idFromServer, phoneInfo[] phonesBLE){
        if(idFromServer.length==this.PhoneIDSizeFromServer) {
            ...
            for (int i = 0; i < this.PhoneIDSizeFromServer; i++) {
                System.out.println(TAG+"ID FROM SERVER : "+idFromServer[i]);
           ...
           System.out.println(TAG+"SIZE NOT CORRECT");
        }
    }
    ...
    public void setVirtualID() {
        this.PhoneIDFromServer = new String[]{"BLE01","PINGPONG","hj","juha"};
        this.PhoneIDSizeFromServer = PhoneIDFromServer.length;
    }
